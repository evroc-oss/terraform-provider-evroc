...
func TestExpandSecurityGroupRulesRemoteSecurityGroup(t *testing.T) {
	ms := newMockServer()
	defer ms.close()
	setupCatchAll(ms)
	...
	t.Run("plain name resolves to FQID", func(t *testing.T) {
		input := []interface{}{
			map[string]interface{}{
				"name":                  "allow-from-lb",
				"direction":             "Ingress",
	...
	t.Run("FQID passes through", func(t *testing.T) {
