...
# --- Security Group ---
# An HA k3s control plane needs more than the API port:
#   6443  Kubernetes API (clients, kubectl, node registration)   -> public
#   9345  k3s supervisor / server registration                   -> public
#   2379  embedded etcd client   } required BETWEEN control       -> cluster only
#   2380  embedded etcd peer     } planes, or HA can never form   -> cluster only
#   10250 kubelet API (kubectl logs/exec, metrics-server)         -> cluster only
# etcd/kubelet are scoped to the private cluster CIDR rather than the internet.
...
# --- First control plane: founds etcd and is the fixed registration address ---
...
# --- Remaining control planes: join the first node directly ---
# New nodes join an explicit existing member (the first control plane's private
# IP), which is the standard approach for clustered systems. After joining, k3s
# discovers every apiserver and load-balances across them client-side, so these
# nodes are not pinned to the founder at runtime.
#
# (They live in their own resource rather than one count-based block because a
# count instance cannot reference a sibling instance's IP — Terraform would see
# a cycle.)
...
# --- Workers: also register against the first control plane directly ---
